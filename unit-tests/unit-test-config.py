...
def generate_cmake_cpp( builddir, testdir, testname, filelist ):
...
def generate_cmake_py( builddir, testdir, filelist ):
    makefile = builddir + '/' + testdir + '/CMakeLists.txt'
    debug( '   creating:', makefile )
    handle = open( makefile, 'w' )
    filelist = '\n    '.join( filelist )
    ...
    cmake_minimum_required( VERSION 3.1.0 )
    project( ''' + os.path.basename( testdir ) + '''_scripts )
...
def process_cpp( dir, builddir ):
    ...
    for f in find( dir, '(^|/)test-.*\.cpp$' ):
        ...
        generate_cmake_cpp( builddir, testdir, testname, filelist )
        if static:
...
def process_py( dir, builddir ):
    found = []
    for f in find( dir, '(^|/)test-.*\.py$' ):
        testparent = os.path.dirname( f )
        os.makedirs( builddir + '/' + testparent, exist_ok = True )
        if testparent in found:
            continue
        scripts = []
        for script in find( dir + os.sep + testparent, '(^|/)test-.*\.py$' ):
            scripts.append( script )
        generate_cmake_py( builddir, testparent, scripts )
        found.append( testparent )
    return found,[],[]
